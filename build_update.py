...
def copy_in_ab_process(patch_process, src_image_class, need_copy_blocks,
    ...
    if OPTIONS_MANAGER.ab_partition_update:
        ...
        for src_block in range(0, src_image_class.total_blocks):
            # Find each block in no_copy_blocks_in_ab compared to total_blocks
            if src_block not in no_copy_blocks_in_ab:
                need_copy_blocks.append(src_block)
            else:
                non_continuous_blocks.append(src_block)
...
def increment_image_processing(
    ...
    for each_img_name in OPTIONS_MANAGER.incremental_img_name_list:
        ...
        with open(each_tgt_image_path, 'rb') as f:
                    tartget_content = f.read()
        OPTIONS_MANAGER.diff_image_new_data[each_img] = tartget_content
        ...
        if not src_generate_map or not tgt_generate_map:
            if OPTIONS_MANAGER.stream_update:
                ...
                OPTIONS_MANAGER.no_map_file_list.append(each_img_name)
